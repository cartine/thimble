...
`list` → `render`. Source cast at [`assets/demo.cast`](assets/demo.cast)
for `asciinema play` or copy-paste-friendly inspection. The recording is
generated by [`scripts/demo.sh`](scripts/demo.sh), which provisions a
throwaway age identity in a temp dir, runs the canonical lifecycle, and
removes the temp dir on exit. To re-record:
